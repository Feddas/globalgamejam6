...
{	
	private void loadTextHouseItem()
	{
		...
		//Completion.Start
		TextHouseItem.Add(Tuple.Create(HouseItemType.FrontHouseCar, Completion.Start), new List<string>{
		...
		TextHouseItem.Add(Tuple.Create(HouseItemType.LivingroomCodexBox, Completion.Start), new List<string>{
		...
		TextHouseItem.Add(Tuple.Create(HouseItemType.FrontHouseCar, Completion.PlacedUrn), new List<string>{
			"I should be able to leave, but the game developer was too lazy to make alternate endings. I should go back and poke around for ways to open that box on the fireplace mantle."});
		...
		//Completion.Cryptex1Livingroom
		TextHouseItem.Add(Tuple.Create(HouseItemType.LivingroomPaintingSisters, Completion.Cryptex1Livingroom), new List<string>{
			"I wonder when my sister even found the time to make this painting."});
		TextHouseItem.Add(Tuple.Create(HouseItemType.LivingroomCodexBox, Completion.Cryptex1Livingroom), new List<string>{
		...
		//Completion.Cryptex2Hallway
		TextHouseItem.Add(Tuple.Create(HouseItemType.MasterbedMirrorShard, Completion.Cryptex2Hallway), new List<string>{
			"This shard of glass looks sharp. I should clean up this room before I chance getting bloody."});
		TextHouseItem.Add(Tuple.Create(HouseItemType.MasterbedPillow1, Completion.Cryptex2Hallway), new List<string>{
		...
		//Completion.Cryptex3Masterbed
		TextHouseItem.Add(Tuple.Create(HouseItemType.MasterbedMirrorShard, Completion.Cryptex3Masterbed), new List<string>{
		...
		//Completion.Cryptex4MirrorShard
		TextHouseItem.Add(Tuple.Create(HouseItemType.AtticRockingHorse, Completion.Cryptex4MirrorShard), new List<string>{
			"Guess it just needed a touch of reality to stop moving."});

		//Completion.BunnyFell
...
Strange to have an unfinished inscription on the urn though. All it says is, ""YOU ARE""."});
		...
		TextRoom.Add(Tuple.Create(Room.Bedroom, Completion.Cryptex1Livingroom), new List<string>{
			@"This bedroom sure is creaky. I just hope the floor or ceiling doesn't fall out."});
		TextRoom.Add(Tuple.Create(Room.Bedroom, Completion.Cryptex2Hallway), new List<string>{
			@"More things are sure to fall from the ceiling just by stepping in my old bedroom."});
		TextRoom.Add(Tuple.Create(Room.Hallway, Completion.Cryptex2Hallway), new List<string>{
			@"...or new doors unlock."});
		TextRoom.Add(Tuple.Create(Room.Hallway, Completion.Cryptex4MirrorShard), new List<string>{
			@"Those stairs must go up to the attic."});
		TextRoom.Add(Tuple.Create(Room.Attic, Completion.Cryptex4MirrorShard), new List<string>{
